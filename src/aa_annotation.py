...
def aa_annotation(pb_tree:str, ref_fasta:str, gtf:str, output:str):
    '''
    Annotate amino acids in protobuff tree file
    :params: protobuff tree filepath, reference fasta filepath, gtf file path, output directory filepath
    :return: amino acid table (tsv) file path
    '''
    # check_dependency_installed('matUtils')
    aa_fn = str('.'.join(os.path.basename(pb_tree).split('.')[:-1])+('.aa_annotations.tsv'))
    aa_fp = os.path.join(output, aa_fn)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Run Process
    aa_fp = aa_annotation(args.pb_tree, args.reference, args.gtf, args.output)
